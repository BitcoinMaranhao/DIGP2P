...
async def trade_crypto_digpix(valor: float, par: str, is_buy: bool) -> dict:
    """Realiza compra com taxa fixa de 4% para Digpix."""
    try:
        cotacao = await get_binance_price(par)
        taxa = 4  # Taxa fixa de 4%
        fee_service_brl = (valor * taxa) / 100
...
# Função específica para calc_sats_digpix
async def buy_btc_digpix(valor: float): return await trade_crypto_digpix(valor, 'BTCBRL', is_buy=True)